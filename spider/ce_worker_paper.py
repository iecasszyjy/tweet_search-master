...
if __name__ == '__main__':
    ...
    while True:
        queue = r.lpop('task:paper')
        if queue:
            print 'craw_worker process!'
            craw = run_task(json.loads(queue))
            if type(craw) != str:
                db.paper_log.insert_one({'message':json.loads(queue),'status':1})
            else:
                db.paper_log.insert_one({'message':json.loads(queue),'status':0,'error':craw})
        time.sleep(1)
