...
def advance_search_summit(q, f, num, s, u):
...
def run_summit_task(message_data):
    try:
        q = message_data['q']
        ...
        if type(message_data['f']) != list:
            advance_search_summit(q, message_data['f'], num, sinceTimeStamp, untilTimeStamp)
        else:
            pool = Pool(processes=multiprocessing.cpu_count())
            [pool.apply(advance_search_summit, (q, f, num, sinceTimeStamp, untilTimeStamp)) for f in message_data['f']]
            pool.close()
            pool.join()
    except Exception, e:
...
if __name__ == '__main__':
    ...
    while True:
        queue = r.lpop('task:online')
        if queue:
            print 'craw_worker process!'
            try:
                run_summit_task(json.loads(queue))
                db.summit_log.insert_one({'message': json.loads(queue), 'status': 1})
            except Exception, e:
                db.summit_log.insert_one({'message': json.loads(queue), 'status': 0, 'error': e.message})
        # else:
