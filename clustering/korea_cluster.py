...
def get_close_centroids(ids,X,clusters=None,cluster_centers_=None,n=3):
	docs = defaultdict(list)
	for idx,label in enumerate(clusters):
		docs[label].append((ids[idx],nlp(X[idx])))
	distance = defaultdict(list)
	num_clusters = cluster_centers_.shape[0]
	for index in range(num_clusters):
		distance[str(index)] = sorted(list(set([(doc[0],doc[1].text,str(cosine(doc[1].vector,cluster_centers_[index]))) for doc in docs[index]])),key=lambda x:x[2])[-n:]
	return distance
		...
		while if_continue:
			...
			if if_continue:
				if if_continue < 2:
					...
					{'$set': {'entity':ners[index],'cluster':{'cluster_label':0,'cluster_hash':cluster_hash}}}) for index,_id in tqdm(enumerate(ids))]
				...
				requests = [UpdateOne({'_id': _id,'cluster':None}, 
					{'$set': {'entity':ners[index],'cluster':{'cluster_label':clusters[index],'cluster_hash':cluster_hash}}}) for index,_id in tqdm(enumerate(ids))]
				result = db.korea.bulk_write(requests)
				pprint(result.bulk_api_result)
...
def clustering():
	...
	clusters = km.labels_
	clusters = [int(i) for i in clusters]
	centroids = get_close_centroids(ids=ids,X=texts,clusters=clusters,cluster_centers_=cluster_centers_,n=3)
	requests = [UpdateOne({'_id': _id,'cluster':None}, 
		{'$set': {'entity':ners[index],'cluster':{'cluster_label':clusters[index],'cluster_hash':cluster_hash}}}) for index,_id in tqdm(enumerate(ids))]
	result = db.korea.bulk_write(requests)
	pprint(result.bulk_api_result)
